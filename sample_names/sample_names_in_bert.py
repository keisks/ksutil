...
class NameSampler:
    def __init__(self, dist=False, gender=False, year_from=1880, year_to=2017, name_path=""):
        ...
        self.bert_vocab = {}
        for line in open(os.path.join(*[self.name_path, "bert_vocab.tsv"]), 'r'):
            idx, word = line.strip().split('\t')
            self.bert_vocab[word] = idx
        ...
        for y in range(self.year_from, self.year_to+1):
            ...
            for line in open(self.year_file):
                ...
                if line[0].lower() in self.bert_vocab.keys():
                    if self.gender:
                        if line[1] == 'F':
                            if self.dist:
...
if __name__ == "__main__":
    ...
    ns = NameSampler(args.arg_dist, args.arg_gender, args.arg_from, args.arg_to,)
    #print(ns.sample(args.arg_num))
    #print(ns.sample(args.arg_num))
    random.seed(12345)

    names_mf = ns.sample(args.arg_num)
    for names in names_mf:
